...
// This shader animates a wave effect by modifying vertex positions vertically 
// based on a sine wave function. The wave's behavior is controlled using 
// adjustable properties: frequency (_Freq), speed (_Speed), and amplitude (_Amp). 
// The fragment shader applies a texture (_MainTex) and a base color (_BaseColor).
